...
NORMAL_FILE = File.read('data/normal/normal.txt')
UNBALANCE_FILE = File.read('data/unbalance/unbalance.txt')
...
class Kmeans
  ...
  def generate_random_clusters
    ...
    @data_points.sort_by{rand}[1..@k].each do |random_data_point|
      clusters.push Cluster.new(random_data_point)
    end
    clusters
  end

  def reset_cluster_data_points
    @clusters.each { |cluster| cluster.data_points = [] }
  end
  ...
  def calculate_centroids
    @clusters.each { |cluster| cluster.calculate_centroid }
  end
end

if ARGV.size != 2
  puts "USAGE:\nnormal: ruby clusterer.rb data/normal/normal.txt/ K\unbalance: ruby clusterer.rb data/unbalance/unbalance.txt/ K"
else
  srand(999)
